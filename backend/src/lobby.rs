use crate::messages::*;
use actix::{prelude::Context, Actor, Handler, Recipient};
...
impl Handler<ConnectMsg> for Lobby {
    ...
    fn handle(&mut self, msg: ConnectMsg, _: &mut Self::Context) -> Self::Result {
        ...
        self.rooms
            ...
            .for_each(|user_id| {
                self.send_message(
                    &(serde_json::json!(
                        {"req_type" : "JoinReq",
                        "msg" : format!("{} just joined", msg.self_id)}
                    )
                    .to_string()),
                    user_id,
                )
        ...
        self.sessions.insert(msg.self_id, msg.rec_addr);

        let join_msg = serde_json::json!({"req_type" : "MsgOnly", "msg" : format!("Your Socket-id is: {}", msg.self_id)});
        self.send_message(&join_msg.to_string(), &msg.self_id);
    }
}

///
/// when someone gets disconnected from the room
/// send 'user_disconnected' msg to other member...
///
...
impl Handler<ClientActorMsg> for Lobby {
    ...
    fn handle(&mut self, msg: ClientActorMsg, _: &mut Self::Context) -> Self::Result {
        // println!("Step 3 -> Broadcast ClientMSG in Lobby");
        if msg.msg.starts_with("\\w") {
            if let Some(id_to) = msg.msg.split(' ').collect::<Vec<&str>>().get(1) {
                self.send_message(
                    &msg.msg,
                    &Uuid::parse_str(id_to).expect("Failed to parse string to Uuid"),
                )
            }
        } else {
            self.rooms
